    ...
    <div class="container py-4">
        ...
        <div class="mb-2">
            <button class="btn btn-danger"
                    ...
                    data-bs-target="#statuses-filters"
                    aria-expanded="true"
                    aria-controls="statuses-filters">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                window.rememberCollapse('statuses-filters', 'conservationStatusesFiltersOpen');
            });
        </script>

        <div class="collapse" id="statuses-filters">
            <div class="bg-danger p-2 border border-3 rounded-3">
            <form action="{{ route('admin.conservationStatuses.index') }}" method="GET">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover align-middle">
                <thead>
                    <tr>
                        <th title="ordina per ID">{!! sortLink('conservationStatuses', 'ID', 'id', $sort, $direction) !!}</th>
                        <th class="text-center">Immagine</th>
                        <th title="ordina per Nome">{!! sortLink('conservationStatuses', 'Nome', 'name', $sort, $direction) !!}</th>
                        <th title="ordina per Colore">{!! sortLink('conservationStatuses', 'Colore', 'color', $sort, $direction) !!}</th>
                ...
                <tbody>
                    @foreach ($conservationStatuses as $conservationStatus)
                        <tr class="text-center">
                            <td>{{ $conservationStatus->id }}</td>

                            <td>
                                <x-icon :entity="$conservationStatus" measure=60 shape=0 bgPresent=0></x-icon>
                            </td>

                            <td>
                                <strong>{{ $conservationStatus->name }}</strong>
                            </td>
                            ...
                            <td class="text-end text-nowrap w-25">
                                ...
                                <a href="{{ route('admin.conservationStatuses.edit', $conservationStatus) }}" class="btn btn-sm btn-outline-warning">
        ...
        <div class="d-flex justify-content-between align-items-center mt-1 flex-wrap">
            <div class="text-muted small">
                Visualizzati
                {{ $conservationStatuses->firstItem() ?? 0 }}
                -
            ...
            <div>
                {{ $conservationStatuses->links('vendor.pagination.bootstrap-5') }}
            </div>
        </div>
